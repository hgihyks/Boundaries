...
export default function Information() {
  ...
  const internetBankingUsername = `${firstName.toLowerCase()}.karma2020`;
  const staticRows = useMemo(
    () => [
      ...
      ['UPI ID', `${firstName.toLowerCase()}@karma`],
      ['UPI PIN', '4687'],
  ...
  const formatRbc = (totalCells) => {
    const n = Math.max(0, Math.round(Number(totalCells) || 0));
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (modelRef.current) {
        ...
        setRbc(formatRbc(res.RBC_total));
      }
      ...
      if (wbcModelRef.current) {
        ...
        setWbc(formatInt(resW.WBC_total));
      }
      ...
      if (pltModelRef.current) {
        ...
        setPlatelets(formatInt(resP.PLT_total));
      }
    }, 300);
